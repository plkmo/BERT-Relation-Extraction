...
def load_state(net, optimizer, scheduler, args, load_best=False):
    ...
    base_path = "./data/"
    amp_checkpoint = None
    checkpoint_path = os.path.join(base_path, "task_test_checkpoint_%d.pth.tar" % args.model_type)
    best_path = os.path.join(base_path, "task_test_model_best_%d.pth.tar" % args.model_type)
...
def load_results(model_type=0):
    ...
    losses_path = "./data/task_test_losses_per_epoch_%d.pkl" % model_type
