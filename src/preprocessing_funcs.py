...
def load_dataloaders(args, max_length: int = 50000):
    ...
    if not os.path.isfile(f'./data/{cached_pretrain_data_filename}'):
        ...
        text: str = process_textlines(texts)
        ...
        text_chunks: Generator[str] = (text[i * max_length:(i * max_length + max_length)] for i in range(num_chunks))
...
def process_textlines(texts: List[str]) -> str:
    texts = [process_sent(sent) for sent in texts]
    texts_concatenated = " ".join([t for t in texts if t is not None])
    texts_concatenated_extra_whitespaces_stripped: str = re.sub(' {2,}', ' ', texts_concatenated)  # remove extra spaces > 1

    return texts_concatenated_extra_whitespaces_stripped
...
# TODO use dataclass
RelationStatement = Tuple[List[str], Tuple[int, int], Tuple[int, int]]
...
def create_pretraining_corpus(raw_text: str, window_size: int = 40, spacy_model_name: str = 'en_core_web_lg'):
    ...
    for i in tqdm(range(len(ents))):
        ...
        for j in range(1, len(ents) - i):
            e2: spacy.tokens.span.Span = ents[i + j]
            ...
            if (1 <= (e2.start - e1.end) <= window_size):  # check if next nearest entity within window_size
                # TODO use Spacy or NLTK sentence tokenizer
                ...
                tokens: List[str] = [token.text for token in sents_doc[left_r:right_r]]
                ...
                assert e1.start != e1.end
                assert e2.start != e2.end
                assert (e2.start - e1.end) > 0

                r: tuple = (tokens, (e1.start - left_r, e1.end - left_r), (e2.start - left_r, e2.end - left_r))
    ...
    D_tree_parsing, entitys_tree_parsing = relation_staements_from_dependency_tree_parsing(sents_doc, window_size)

    D.extend(D_tree_parsing)
    ents_list.extend(entitys_tree_parsing)
...
def skip_entity(span: spacy.tokens.span.Span) -> bool:
    # why no numbers?
    entities_of_interest = ["EVENT",
                            "FAC",
                            "GPE",
                            "LANGUAGE",
                            "LAW",
...
def relation_staements_from_dependency_tree_parsing(sents_doc: spacy.tokens.doc.Doc, window_size: int):
    doc_sents: List[spacy.tokens.span.Span] = [s for s in sents_doc.sents]
...
class PretrainDataset(Dataset):
    def __init__(self, args, D, batch_size=None):
        ...
        self.E1_token_id: int = self.tokenizer.encode("[E1]")[1:-1][0]
        self.E1s_token_id: int = self.tokenizer.encode("[/E1]")[1:-1][0]
        self.E2_token_id: int = self.tokenizer.encode("[E2]")[1:-1][0]
        self.E2s_token_id: int = self.tokenizer.encode("[/E2]")[1:-1][0]
